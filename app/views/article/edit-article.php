...
<div class="bg-white min-h-screen pb-20">
    ...
    <form action="/article/edit-article" method="POST" enctype="multipart/form-data" class="max-w-5xl mx-auto">
        <!-- Hidden ID -->
        ...
        <div class="sticky top-0 z-30 bg-white/95 backdrop-blur-sm border-b border-slate-100 py-4 px-4 sm:px-6 lg:px-8 flex justify-between items-center">
            <div class="flex items-center gap-4">
                ...
                <span class="text-sm font-medium text-slate-500">Edit Artikel</span>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <button type="submit" class="px-5 py-2 text-sm font-medium bg-blue-600 text-white hover:bg-blue-700 rounded-lg shadow-sm hover:shadow transition-all">
                    Update Artikel
                </button>
            </div>
        </div>
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-12 max-w-4xl mx-auto space-y-8">
            ...
            <div class="group relative aspect-video bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl overflow-hidden hover:border-slate-300 transition-colors flex items-center justify-center cursor-pointer">
                ...
                <div class="text-center p-6 <?= !empty($row['gambar']) ? 'hidden' : '' ?>">
                    ...
                    <p class="text-sm font-medium text-slate-600">Ganti Cover Image</p>
                    <p class="text-xs text-slate-400 mt-1">Klik atau drag gambar ke sini</p>
                </div>
                
                <img src="<?= !empty($row['gambar']) ? '/image?file=' . urlencode($row['gambar']) : '#' ?>" alt="Preview" class="<?= !empty($row['gambar']) ? '' : 'hidden' ?> absolute inset-0 w-full h-full object-cover">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="flex">
                        ...
                        <input type="text" name="slug" value="<?= htmlspecialchars($row['slug']) ?>" class="rounded-none rounded-r-lg bg-slate-50 border border-slate-200 text-slate-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm p-2.5 outline-none transition-all" placeholder="url-slug">
                    </div>
                </div>
            </div>
...
<div class="max-w-none">
                <textarea
                    ...
                    placeholder="Tulis ceritamu di sini..."><?= htmlspecialchars($row['content']) ?></textarea>
            </div>
